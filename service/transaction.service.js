...
class TransactionService {
  ...
  async getAll(userId, reception = null, isProfit) {
    ...
    switch (reception) {
     case 'all':
        data = await TransactionModel.find({userId}).sort({ dateTransaction: -1 }).sort({ createdAt: -1})
        break
     case 'profit':
        data = await TransactionModel.find({userId, isProfit: true}).sort({ dateTransaction: -1}).sort({ createdAt: -1})  
        break
     case 'costs':
        data = await TransactionModel.find({userId, isProfit: false}).sort({ dateTransaction: -1 }).sort({ createdAt: -1})
        break
      default:
        data = await TransactionModel.find({userId}).sort({ dateTransaction: -1 }).sort({ createdAt: -1})
    }
